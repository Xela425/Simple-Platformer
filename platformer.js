(function() { // module pattern
  //Settings
  var requireMonsterKills = false;
      resetMapOnPlayerDeath = true;
      //makes collecting 1 treasure change the map
      debugMode = false;
  ...
  var MAP      = { tw: 64, th: 48 },
      ...
      KEY      = { SPACE: 32, LEFT: 37, UP: 38, RIGHT: 39, DOWN: 40 , A: 65, D: 68, W: 87, S: 83, };
  ...
  var fps      = 60,
      ...
      treasure = [],
      cells    = [],
      levelList = ["level01.json", "level02.json", "level03.json", "level04.json", "level05.json", "level06.json"],
      levelNum = 1;
  ...
  function checkFinished() {
    if ( player.collected == treasure.length && treasure.length >= 0) {
      if (requireMonsterKills) { 
  ...
  function resetMap() {
    ...
    if (levelNum > levelList.length) {
      levelID = "YouWin.json";
      levelNum = levelList.length + 1;
    } else {
      levelID = levelList[levelNum - 1];
    }
    console.log("changing to " + levelID + " levelNum now equals " + levelNum );
    get(levelID, function(req) {
  ...
  function killPlayer(player) {
    ...
    if(resetMapOnPlayerDeath) {resetMap();}
  }
  ...
  function setupEntity(obj) {
    var entity = {};
    entity.x        = obj.x - (obj.x % 32);
    entity.y        = obj.y - (obj.y % 32);
    entity.dx       = 0;
    entity.dy       = 0;
    ...
    if (obj.properties != null) {
      for(var i = 0; i < obj.properties.length; i++) {
        switch(obj.properties[i].name) {
          case "gravity" : entity.gravity = METER * (obj.properties[i].value);
          case "maxdx"   : entity.maxdx   = METER * (obj.properties[i].value);
          case "maxdy"   : entity.maxdy   = METER * (obj.properties[i].value);
          case "impulse" : entity.impulse = METER * (obj.properties[i].value);
          //case "left"    : entity.left    = true;
          //case "right"   : entity.right   = true;
    }}}
